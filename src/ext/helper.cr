...
def __sort_ivars__(s : String) : String
  # Foo(@major=1, @build=nil)
  case s
  when /^([A-Z][a-zA-Z0-9_]*(::[A-Z][a-zA-Z0-9_]*)*)\(@(.*)\)$/
    class_name = $1                      # => "Foo"
    definition = $3                      # => "major=1, @build=nil"
    fields     = definition.split(/, @/) # ["major=1", "build=nil"]
    sorted     = fields.sort.map{|i| "@#{i}"}
    buffer     = "%s(%s)" % [class_name, sorted.join(", ")]
    return buffer
  else
    raise ArgumentError.new("__sort_ivars__ can't parse #{s.inspect}")
  end
end
